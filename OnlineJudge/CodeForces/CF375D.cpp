...
{
	for(int i=L[u];i<=R[u];++i)--res[cnt[A[Re[i]]]--];
}
...
{
	for(int i=head[u],v;v=to[i],i;i=nxt[i])
		if(v!=fa[u]&&v!=son[u])_dfs(v),Del(v);
	if(son[u])_dfs(son[u]);
	for(int i=head[u],v;v=to[i],i;i=nxt[i])
...
int main()
{
	...
	for(int i=1,u;i<=m;++i)scanf("%d%d",&u,&B[i]),Q[u].push_back(i);
	dfs(1,0);_dfs(1);for(int i=1;i<=m;++i)printf("%d\n",ans[i]);
}