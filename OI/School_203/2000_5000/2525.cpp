#include<bits/stdc++.h>
using namespace std;
const int maxn=3e4+5,maxm=6e5+5;
#define mid (L+R>>1)
...
void Insert(int L,int R,int x,int ot,int &rt)
{
	rt=++tot;cnt[rt]=cnt[ot]+1;
	ls[rt]=ls[ot];rs[rt]=rs[ot];
	if(L==R)return;
	if(x<=mid)Insert(L,mid,x,ls[ot],ls[rt]);
	else Insert(mid+1,R,x,rs[ot],rs[rt]);
}
...
int main()
{
	...
	for(int i=1;i<=n;++i)
		...
		Insert(1,len,A[i],tree[i-1],tree[i]);
	while(m--)
	{
		int L,R,k;
		scanf("%d%d%d",&L,&R,&k);
		printf("%d\n",B[query(1,len,tree[L-1],tree[R],R-L+2-k)]);
	}
}