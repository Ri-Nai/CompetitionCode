...
namespace T
{
    ...
    void dfs_(int u,int f,int t)
    {
        ...
        B[u]+=B[f];
        tp[u]=t;
        if(son[u])dfs_(son[u],u,t);
        for(auto [v,w,id]:P[u])
        {
            if(v==f)continue;
            if(v!=son[u] and !vis[v])dfs_(v,u,v);
        }
    }
    int LCA(int u,int v)
    {
    ...
    void solve()
    {
        ...
        for(int u=1;u<=n;++u)
            for(auto [v,w,id]:E[u])
                if(blg[u]==blg[v])A[blg[u]]|=w;
                else P[blg[u]].push_back({blg[v],w,id});
        dfs(1,0);
        for(int i=1;i<=blk;++i)vis[i]=0;
        dfs_(1,0,1);
        for(int u,v;q--;)
        {
            u=blg[rd()];
            ...
            if(a|b)cout<<"wow!golden legendary!\n";
            else cout<<"awsl!\n";
        }
    }
}
